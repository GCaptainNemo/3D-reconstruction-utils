...
namespace ICP_set
{
	std::map<std::string, ICPFunPtr> icp_map
	{ {"icpPlaneToPlane", &ICP_set::icpPlaneToPlane},
	...
	int demo(const std::string & pcd_source_path, const std::string &pcd_target_path, const std::string & option) {
		...
		// clang-format off
		// clang-format on
		Eigen::Matrix4f transformation = run(cloud_source, cloud_target, icp_map[option]);
	...
	/*icp_map.insert({ "icpPlaneToPlane", &ICP_set::icpPlaneToPlane });
	icp_map.insert({ "icpPlaneToPlane", &ICP_set::icpPlaneToPlane });
	icp_map.insert({ "icpPlaneToPlane", &ICP_set::icpPlaneToPlane });
	icp_map.insert({ "icpPlaneToPlane", &ICP_set::icpPlaneToPlane });*/
